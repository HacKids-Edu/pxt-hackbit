...
namespace hackbitIR {
  ...
  const MICROBIT_HACKBIT_IR_NEC = 777;
  const MICROBIT_HACKBIT_IR_DATAGRAM = 778;
  const MICROBIT_HACKBIT_IR_BUTTON_PRESSED_ID = 789;
  const MICROBIT_HACKBIT_IR_BUTTON_RELEASED_ID = 790;
  const IR_REPEAT = 256;
  ...
  function enableIrMarkSpaceDetection(pin: DigitalPin) {
    ...
    pins.onPulsed(pin, PulseValue.High, () => {
      ...
      if (status !== IR_INCOMPLETE) {
        control.raiseEvent(MICROBIT_HACKBIT_IR_NEC, status);
      }
    });
  }
  ...
  //% blockId="hackbit_infrared_connect_receiver"
    ...
    control.onEvent(
      ...
      () => {
        ...
        if (irEvent === IR_DATAGRAM) {
          ...
          control.raiseEvent(MICROBIT_HACKBIT_IR_DATAGRAM, 0);
          ...
          if (newCommand !== activeCommand) {
            if (activeCommand >= 0) {
              control.raiseEvent(
                MICROBIT_HACKBIT_IR_BUTTON_RELEASED_ID,
                activeCommand
              );
            }
            ...
            control.raiseEvent(
              MICROBIT_HACKBIT_IR_BUTTON_PRESSED_ID,
              newCommand
            );
          }
        }
   ...
   */
  //% subcategory="IR Receiver"
  //% blockId=hackbit_infrared_on_ir_button
   ...
   */
  //% subcategory="IR Receiver"
  //% blockId=hackbit_infrared_button_code
  //% button.fieldEditor="gridpicker"
  //% button.fieldOptions.columns=3
  //% button.fieldOptions.tooltips="false"
