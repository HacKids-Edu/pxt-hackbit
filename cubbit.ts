 ...
 */
//% weight=100 color=#00C6F5 icon="\uf544" block="Hackbit cub:bit"
namespace cubbit {
    export enum LedLr {
        //% block="A button"
        AButtonSide = 8,
    ...
    */
    ...
    function doMotion(fileName: number, flameNum: number) {
        ...
        while (1) {
            ...
            if(cubbitDebug)serial.writeString(",>OK");
            listNum += 1; // >
            if ( 0x4d46 != ( mBuf[listNum] << 8 | mBuf[listNum + 1] ) )break;
            if(cubbitDebug)serial.writeString(",mfOK");
            listNum += 2; // MF
            if ( fileName != ( num2Hex(mBuf[listNum]) << 4 | num2Hex(mBuf[listNum + 1]) ) )break;
            if(cubbitDebug)serial.writeString(",fileOK");
            ...
            for (let val = 0; val < SERVO_NUM_USED; val++){
                ...
                data[val] = numHex;
                if(cubbitDebug)serial.writeNumber(data[val]);
                listNum += 4;
            }
            setAngle(data, time);
            if(!loopBool)break;
